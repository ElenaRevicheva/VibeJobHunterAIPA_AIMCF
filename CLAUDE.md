...
> Auto-generated by CTO AIPA on 2026-03-27 via full codebase scan. Refreshed 2026-05-26 (header note: Bright Data cross-repo signal).
> Working dir: `D:\aideazz\VibeJobHunterAIPA_AIMCF`
> Production: Oracle Cloud VM (us-chicago-1), systemd `vibejobhunter`

> **2026-05-26 cross-repo signal — Bright Data enrichment flows through here too.**
> The `cto-aipa` repo (https://github.com/ElenaRevicheva/AIPA_AITCF) shipped 4 Bright Data primitives in `src/brightdata-enrich.ts` (`bdFetch`, `bdSerpSearch`, `bdScrapingBrowserFetch`, `bdSmartFetch`). VJH's LinkedIn Jobs discovery loop already calls Web Unlocker via `bdFetch` for enrichment at 120 jobs/cycle — that integration is the OLDEST production use of Bright Data in the AIdeazz ecosystem. The hackathon-week work (May 25–26) added 3 more BD products in cto-aipa and built `src/research-agent.ts` (autonomous Claude tool-use loop). VJH gets two indirect wins:
> 1. Same shared Bright Data credential set across the ecosystem — no separate token rotation pain when keys roll.
> 2. New `/research_employer <company>` Telegram command in cto-aipa is the operator-facing complement to VJH's autonomous discovery. When VJH surfaces a hiring lead, Elena can fire `/research_employer` and get a sendable application angle in ~90 seconds, then route the lead into the HubSpot hiring pipeline VJH already writes to. This addresses the long-standing "VJH applies, but Elena has no fast way to research the human-loop opportunities VJH found" gap noted in `docs/CAREER_FOCUS.md` §0.
>
> Bright Data Web Data UNLOCKED hackathon (lablab.ai, May 26) submission shipped — full proof in `aideazz` repo `pitch-pdf/aideazz-pitch-brightdata-hackathon-2026-05-26.pdf` (commit `f0c8a26`) and in `aideazz-private-docs/docs/01-career-applications/Accelerator-Applications/BrightData-WebDataUnlocked-2026/` (commit `45583e0`).
...
## 🆕 May 20 2026 update — HubSpot dealname prefix (sourcePrefix)

VJH writers now stamp `sourcePrefix` into HubSpot CRM event payloads:

- `src/langgraph_pipeline/crm_hub.py` → sends `"sourcePrefix": "HIRING-VJH"` with each LangGraph application push.
- `src/search/serpapi_jobs_ingest.py` → sends `"sourcePrefix": "HIRING-VJH-SERP"` with each Google Jobs API result.

The backend (`cto-aipa.ts` `/api/crm-event` endpoint) wraps the dealname as `[HIRING-VJH] <jobTitle> @ <company>` (or `[HIRING-VJH-SERP] ...`), making Elena's HubSpot dashboard scannable by source.
