"""
Google Analytics 4 Dashboard API Routes
Add these to your FastAPI app in Railway
"""
...
async def get_dashboard(days: int = Query(7, ge=1, le=90)):
    """
    Web-based GA4 Dashboard
    
    Access via: https://your-railway-app.railway.app/analytics/dashboard
...
<body>
    <div class="container">
        <div class="header">
            ...
            <button class="refresh-btn" onclick="location.href='?days=7'">7 Days</button>
            <button class="refresh-btn" onclick="location.href='?days=30'">30 Days</button>
            <button class="refresh-btn" onclick="location.href='/analytics/metrics?days={days}'">📊 JSON API</button>
        </div>
...
async def get_metrics(days: int = Query(7, ge=1, le=90)):
    """
    Get GA4 metrics as JSON
    
    Access via: https://your-railway-app.railway.app/analytics/metrics
...
async def health_check():
    ...
    Access via: https://your-railway-app.railway.app/analytics/health
    """
    tracker = GA4PerformanceTracker()
    
    is_connected = tracker.client is not None
    has_property_id = tracker.property_id is not None
    
    return {
        "status": "healthy" if (is_connected and has_property_id) else "degraded",
        "ga4_connected": is_connected,
        "property_id_configured": has_property_id,
        "timestamp": datetime.now().isoformat()
    }
