...
def iron_clad_fit(title: str, location: str, desc: str) -> bool:
    ...
    title_l, loc, desc_l = title.lower(), location.lower(), desc.lower()
    ...
    # ── LATAM/Panama eligibility: TRUST the structured region tag when present
    # (Remotive's candidate_required_location). A specific non-Panama country tag
    # (Brazil-only, USA-only) is authoritative and parks the job even if the
    # description name-drops "Americas". Only fall back to description keywords
    # when there is no clean region tag (e.g. Google Jobs has location=''). ──
