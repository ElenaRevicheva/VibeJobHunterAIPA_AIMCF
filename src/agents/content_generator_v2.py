...
class ContentGeneratorV2:
    ...
    def tailor_resume(self, profile: Profile, job: JobPosting) -> str:
        ...
        # Check cache first
        cached = self.cache.get(prompt, "claude-3-5-sonnet-latest")
        if cached:
        ...
        try:
            ...
            # Cache response
            self.cache.set(prompt, "claude-3-5-sonnet-latest", tailored_resume)
    ...
    def generate_cover_letter(self, profile: Profile, job: JobPosting) -> str:
        ...
        try:
            ...
            # Cache response
            self.cache.set(prompt, "claude-3-5-sonnet-latest", cover_letter)
